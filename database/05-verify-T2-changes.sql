-- Verification Script for T2 Merge Changes
...
-- Weather: 1,123,619 -> 1,125,226 (1,607 new records)
-- Zdarzenie_na_trasie: 56,073 -> 51,575 (net change, some deleted/updated)

PRINT '=== T2 MERGE VERIFICATION ===';
PRINT '';
...
DECLARE @PociagCount INT, @MaszynistaCount INT, @PrzejazdCount INT, @ZdarzenieCount INT,
        @StacjaCount INT, @KursCount INT, @OdcinekCount BIGINT, @WeatherCount BIGINT,
...
PRINT 'Weather:             ' + CAST(@WeatherCount AS VARCHAR(20)) + ' (T1: 1,123,619 -> T2: 1,125,226)';
...
PRINT 'Weather - New records for new Odcinek_kursu:';
SELECT COUNT(*) AS New_Weather_Records
FROM Weather
WHERE id_odcinka > 1123619;
...
PRINT 'Weather conditions distribution (after T2):';
SELECT
    typ_opadow,
    COUNT(*) AS Liczba_Pomiarow,
    CAST(AVG(temperatura) AS DECIMAL(5,2)) AS Srednia_Temperatura,
    CAST(AVG(ilosc_opadow) AS DECIMAL(5,2)) AS Srednia_Ilosc_Opadow
FROM Weather
GROUP BY typ_opadow
...
SELECT
    'Weather' AS Tabela,
    MIN(data_pomiaru) AS Najwczesniejsza_Data,
    MAX(data_pomiaru) AS Najpozniejsza_Data,
    DATEDIFF(DAY, MIN(data_pomiaru), MAX(data_pomiaru)) AS Rozpietosc_Dni
FROM Weather;
...
PRINT '- New Odcinek_kursu with corresponding Weather data';
